...
bsp_status_t bsp_i2c_transmit(bsp_i2c_t i2c, uint8_t addr, uint8_t *data,
  ...
  if (block) {
    return HAL_I2C_Master_Transmit(bsp_i2c_get_handle(i2c), addr, data, size,
                                   10) == HAL_OK
               ? BSP_OK
               : BSP_ERR;
  } else {
    return HAL_I2C_Master_Transmit_DMA(bsp_i2c_get_handle(i2c), addr, data,
                                       size) == HAL_OK
               ? BSP_OK
               : BSP_ERR;
  }
...
bsp_status_t bsp_i2c_receive(bsp_i2c_t i2c, uint8_t addr, uint8_t *buff,
                             size_t size, bool block) {
  if (block) {
    return HAL_I2C_Master_Receive(bsp_i2c_get_handle(i2c), addr, buff, size,
                                  10) == HAL_OK
               ? BSP_OK
               : BSP_ERR;
  } else {
    return HAL_I2C_Master_Receive_DMA(bsp_i2c_get_handle(i2c), addr, buff,
                                      size) == HAL_OK
               ? BSP_OK
               : BSP_ERR;
  }
...
bsp_status_t bsp_i2c_mem_read(bsp_i2c_t i2c, uint8_t addr, uint8_t reg,
  ...
  if (block) {
    return HAL_I2C_Mem_Read(bsp_i2c_get_handle(i2c), addr, reg,
                            I2C_MEMADD_SIZE_8BIT, data, size, 10) == HAL_OK
               ? BSP_OK
               : BSP_ERR;
  } else {
    return HAL_I2C_Mem_Read_DMA(bsp_i2c_get_handle(i2c), addr, reg,
...
bsp_status_t bsp_i2c_mem_write(bsp_i2c_t i2c, uint8_t addr, uint8_t reg,
  ...
  if (block) {
    ...
    return HAL_I2C_Mem_Write_DMA(bsp_i2c_get_handle(i2c), addr, reg,
