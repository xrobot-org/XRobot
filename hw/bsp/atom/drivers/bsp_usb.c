...
bsp_status_t bsp_usb_transmit(const uint8_t *buffer, uint32_t len) {
  bsp_uart_transmit(BSP_UART_USB, (uint8_t *)(buffer), len, false);
...
static void uart_rx_cb(void *arg) {
  ...
  if (!connected) {
    connected = true;
    bsp_uart_abort_receive(BSP_UART_USB);
    bsp_uart_receive(BSP_UART_USB, usb_rx_buff, sizeof(usb_rx_buff), false);
    return;
  }
  usb_buff_len = bsp_uart_get_count(BSP_UART_USB);
  usb_buff_index = 0;
  avail = true;
  bsp_uart_abort_receive(BSP_UART_USB);
  bsp_uart_receive(BSP_UART_USB, usb_rx_buff, sizeof(usb_rx_buff), false);
...
void bsp_usb_init() {
  bsp_uart_register_callback(BSP_UART_USB, BSP_UART_IDLE_LINE_CB, uart_rx_cb,
                             NULL);
  bsp_uart_register_callback(BSP_UART_USB, BSP_UART_TX_CPLT_CB, uart_tx_cb,
                             NULL);
  bsp_uart_receive(BSP_UART_USB, usb_rx_buff, sizeof(usb_rx_buff), false);
