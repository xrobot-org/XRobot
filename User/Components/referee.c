...
void Protocol_DecodePC(Protocol_PcInfoDown_t* hp, const uint8_t *raw) {
	
}

void Protocol_EncodePC(uint8_t* raw, const Protocol_PcInfoUp_t *hp) {
	
}

void Protocol_DecodeJudge(Protocol_JudgeInfo_t* hp, const uint8_t *raw) {
	memcpy(hp, raw, sizeof(Protocol_JudgeInfo_t));
}

void Protocol_EncodeJudge(uint8_t* raw, const Protocol_JudgeInfo_t *hp) {
	memcpy(raw, hp, sizeof(Protocol_JudgeInfo_t));
}

/* Made some modification. Be aware when debug.*/
void Protocol_DecodeRemote(Protocol_RemoteInfo_t* hp, const uint8_t *raw) {
