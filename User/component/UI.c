...
void UI_DrawLine(UI_InterStudent_UIEle_t *grapic_line, const char *name,
                 ...
                 uint16_t y_end) {
  memcpy(grapic_line->name, name, strlen(name) + 1);
...
void UI_DrawRectangle(UI_InterStudent_UIEle_t *grapic_rectangle,
  ...
  memcpy(grapic_rectangle->name, name, strlen(name) + 1);
...
void UI_DrawCycle(UI_InterStudent_UIEle_t *grapic_cycle, const char *name,
  ...
  memcpy(grapic_cycle->name, name, strlen(name) + 1);
...
void UI_DrawOval(UI_InterStudent_UIEle_t *grapic_oval, const char *name,
                 ...
                 uint16_t y_semiaxis) {
  memcpy(grapic_oval->name, name, strlen(name) + 1);
...
void UI_DrawArc(UI_InterStudent_UIEle_t *grapic_arc, const char *name,
                ...
                uint16_t x_center, uint16_t y_center, uint16_t x_semiaxis,
                uint16_t y_semiaxis) {
  memcpy(grapic_arc->name, name, strlen(name) + 1);
...
void UI_DrawFloating(UI_InterStudent_UIEle_t *grapic_floating, const char *name,
  ...
  memcpy(grapic_floating->name, name, strlen(name) + 1);
...
void UI_DrawInteger(UI_InterStudent_UIEle_t *grapic_integer, const char *name,
  ...
  memcpy(grapic_integer->name, name, strlen(name) + 1);
...
void UI_DrawCharacter(UI_Drawcharacter_t *grapic_character, const char *name,
  ...
  memcpy(grapic_character->grapic_t.name, name, strlen(name) + 1);
  ...
  memcpy(grapic_character->character, character, strlen(character) + 1);
}