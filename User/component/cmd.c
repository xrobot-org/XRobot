...
int8_t CMD_Parse(const CMD_RC_t *rc, CMD_t *cmd) {
	...
	/* PC Control. */
	if (CMD_KeyPressed(rc, CMD_KEY_SHIFT) && CMD_KeyPressed(rc, CMD_KEY_CTRL) && CMD_KeyPressed(rc, CMD_KEY_Q))
		cmd->pc_ctrl = true;
	
	if (CMD_KeyPressed(rc, CMD_KEY_SHIFT) && CMD_KeyPressed(rc, CMD_KEY_CTRL) && CMD_KeyPressed(rc, CMD_KEY_E))
		cmd->pc_ctrl = false;
