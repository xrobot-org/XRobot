...
bool CAN_CheckMotorInit(const CAN_MotorInit_t *mi) {
	
	return true;
}

static void CAN_RxFifo0MsgPendingCallback(void) {
	HAL_CAN_GetRxMessage(BSP_CAN_GetHandle(BSP_CAN_1), CAN_MOTOR_CAN_RX_FIFO, &rx_header, motor_rx_data);
...
static void CAN_RxFifo1MsgPendingCallback(void) {
	HAL_CAN_GetRxMessage(BSP_CAN_GetHandle(BSP_CAN_1), CAN_UWB_CAN_RX_FIFO, &rx_header, uwb_rx_data);
...
int8_t CAN_Motor_Control(CAN_MotorGroup_t group, const float *out, uint8_t num) {
	...
	HAL_CAN_AddTxMessage(BSP_CAN_GetHandle(BSP_CAN_1), &tx_header, motor_tx_data, (uint32_t*)CAN_TX_MAILBOX0); 
