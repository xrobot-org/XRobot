...
static bool Gimbal_Steady(Gimbal_t *g) {
  ...
  bool con1 = g->fb.gyro.x < 0.1f;
  bool con2 = g->fb.gyro.y < 0.1f;
  bool con3 = g->fb.gyro.z < 0.1f;
...
int8_t Gimbal_Init(Gimbal_t *g, const Gimbal_Params_t *param, float dt_sec) {
  ...
  g->fb.eulr.imu = BSP_Malloc(sizeof(AHRS_Eulr_t));
...
int8_t Gimbal_UpdateFeedback(Gimbal_t *g, CAN_t *can) {
  ...
  g->fb.eulr.encoder.yaw =
      can->gimbal_motor_fb[CAN_MOTOR_GIMBAL_YAW].rotor_angle;
  g->fb.eulr.encoder.pit =
      can->gimbal_motor_fb[CAN_MOTOR_GIMBAL_PIT].rotor_angle;
...
int8_t Gimbal_Control(Gimbal_t *g, CMD_Gimbal_Ctrl_t *g_ctrl) {
  ...
  switch (g->mode) {
    ...
    case GIMBAL_MODE_ABSOLUTE:
      motor_gyro_set =
          PID_Calc(&(g->pid[GIMBAL_PID_YAW_IN]), g->set.eulr.yaw,
                   g->fb.eulr.imu->yaw, g->fb.gyro.z, g->dt_sec);
      g->out[GIMBAL_ACTR_YAW] =
          PID_Calc(&(g->pid[GIMBAL_PID_YAW_OUT]), motor_gyro_set,
                   g->fb.gyro.z, 0.f, g->dt_sec);

      motor_gyro_set =
          PID_Calc(&(g->pid[GIMBAL_PID_PIT_IN]), g->set.eulr.pit,
                   g->fb.eulr.imu->pit, g->fb.gyro.x, g->dt_sec);
      g->out[GIMBAL_ACTR_PIT] =
          PID_Calc(&(g->pid[GIMBAL_PID_PIT_OUT]), motor_gyro_set,
                   g->fb.gyro.x, 0.f, g->dt_sec);
      break;
    ...
    case GIMBAL_MODE_RELATIVE:
      g->out[GIMBAL_ACTR_YAW] =
          PID_Calc(&(g->pid[GIMBAL_PID_REL_YAW]), g->set.eulr.yaw,
                   g->fb.eulr.encoder.yaw, g->fb.gyro.z, g->dt_sec);
      g->out[GIMBAL_ACTR_PIT] =
          PID_Calc(&(g->pid[GIMBAL_PID_REL_PIT]), g->set.eulr.pit,
                   g->fb.eulr.encoder.pit, g->fb.gyro.x, g->dt_sec);
      break;
  }
  /* Filter output. */
  for (uint8_t i = 0; i < GIMBAL_ACTR_NUM; i++)
