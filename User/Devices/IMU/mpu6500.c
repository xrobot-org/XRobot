...
static void MPU_Read(const uint8_t reg, uint8_t* p_data, uint8_t len) {
	if (p_data == NULL)
		return;
...
static void IST_Write(const uint8_t reg, uint8_t const data) {
	MPU_Write(MPU6500_I2C_SLV4_ADDR, IST8310_ADDRESS);
	MPU_Write(MPU6500_I2C_SLV4_REG, reg);
	MPU_Write(MPU6500_I2C_SLV4_DO, data);
	MPU_Write(MPU6500_I2C_SLV4_CTRL, 0x80);
}

/* Make sure the RESET pin of ist8310 is setted to HIGH or FLOATing. */
static void IST_Read(const uint8_t reg, uint8_t* p_data) {
	if (p_data == NULL)
		return;
	
	MPU_Write(MPU6500_I2C_SLV4_ADDR, 0x80 | IST8310_ADDRESS);
	MPU_Write(MPU6500_I2C_SLV4_REG, reg);
	MPU_Write(MPU6500_I2C_SLV4_CTRL, 0x80);
...
Board_Status_t IMU_Init(IMU_t* himu) {
	...
	Board_Delay(100);
	MPU_Write(MPU6500_SIGNAL_PATH_RESET, 0x0f); /* Reset device */
	Board_Delay(100);
	
	MPU_Write(MPU6500_PWR_MGMT_1, 0x03); /* Clock source -> gyro-z */
	MPU_Write(MPU6500_PWR_MGMT_2, 0x00); /* Enable acc & gyro */
	MPU_Write(MPU6500_CONFIG, 0x04); /* LPF 41Hz */
	MPU_Write(MPU6500_GYRO_CONFIG, 0x18); /* gyro range 0x10:+-1000dps 0x18:+-2000dps */
	MPU_Write(MPU6500_ACCEL_CONFIG, 0x18); /* +-2G */
	MPU_Write(MPU6500_ACCEL_CONFIG_2, 0x02); /* Enable and set acc LPF */
	
	/* IST8310 init. */
	MPU_Write(MPU6500_USER_CTRL, 0x30); /* Enable IIC master mode. */
	MPU_Write(MPU6500_I2C_MST_CTRL, 0x0d); /* Set IIC 400kHz. */
	...
	IST_Write(IST8310_AVGCNTL, 0x12); /* Set low noise mode. */
	IST_Write(IST8310_PDCNTL, 0xc0); /* Set pulse duration normal mode. */
	 
	/* Setup auto read. */

	/* Set slv0 to auto write. */
	MPU_Write(MPU6500_I2C_SLV0_ADDR, IST8310_ADDRESS);
	MPU_Write(MPU6500_I2C_SLV0_REG, IST8310_CNTL1);
	MPU_Write(MPU6500_I2C_SLV0_DO, 0x01);
	
	/* Set slv1 to auto read. */
	MPU_Write(MPU6500_I2C_SLV1_ADDR, (0x80 | IST8310_ADDRESS));
	MPU_Write(MPU6500_I2C_SLV1_REG, IST8310_DATAXL);
	
	/* Enable slv0 and slv1 access delay */
	MPU_Write(MPU6500_I2C_MST_DELAY_CTRL, 0x03);
	
	/* Enable slv0 auto transmit */
	MPU_Write(MPU6500_I2C_SLV0_CTRL, 0x81);
