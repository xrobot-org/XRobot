...
AHRS::AHRS()
  ...
  auto ahrs_thread = [](AHRS *ahrs) {
    ...
    while (1) {
      ...
      ahrs->accl_abs_tp_.Publish(ahrs->accl_abs_);
    }
  };
...
void AHRS::GetABSAccl()
{
    float gravity_b[3];
    static float accl_b[3];
    gravity_b[0]= 2.0f * ((quat_.q1  * quat_.q3  - quat_.q0  * quat_.q2 ) * Gravity);

    gravity_b[1] = 2.0f * ((quat_.q2  * quat_.q3  + quat_.q0  * quat_.q1 ) *Gravity);

    gravity_b[2] = 2.0f * ((0.5f - quat_.q1  * quat_.q1  - quat_.q2  * quat_.q2 ) *Gravity);


     accl_b[0] = this->accl_.x - gravity_b[0];
     accl_b[1] = this->accl_.y - gravity_b[1];
     accl_b[2] = this->accl_.z - gravity_b[2];

    // accl_b[0] = dfilter_.Apply(this->accl_.x - gravity_b[0],dt_);

    // accl_b[1] = dfilter_.Apply(this->accl_.y - gravity_b[1],dt_);
    // accl_b[2] = dfilter_.Apply(this->accl_.z - gravity_b[2],dt_);


   this->accl_abs_.x = 2.0f * ((0.5f - quat_.q2 * quat_.q2 - quat_.q3 * quat_.q3) * accl_b[0] +
                       (quat_.q1 * quat_.q2 - quat_.q0 * quat_.q3) * accl_b[1] +
                       (quat_.q1 * quat_.q3 + quat_.q0 * quat_.q2) * accl_b[2]);

   this->accl_abs_.y = 2.0f * ((quat_.q1 * quat_.q2 + quat_.q0 * quat_.q3) * accl_b[0] +
                       (0.5f - quat_.q1 * quat_.q1 - quat_.q3 * quat_.q3) * accl_b[1] +
                       (quat_.q2 * quat_.q3 - quat_.q0 * quat_.q1) * accl_b[2]);

    this->accl_abs_.z = 2.0f * ((quat_.q1 * quat_.q3 - quat_.q0 * quat_.q2) * accl_b[0] +
                       (quat_.q2 * quat_.q3 + quat_.q0 * quat_.q1) * accl_b[1] +
                       (0.5f - quat_.q1 * quat_.q1 - quat_.q2 * quat_.q2) * accl_b[2]);
...
float AHRS::LowPassFilter(float sample,float dt, float cut_freq_ ,float last_out_ )
{


  float k = 2 * M_2PI * cut_freq_ * dt;

  k = k / (1 + k);

  float out = k * sample + (1 - k) *  last_out_;
