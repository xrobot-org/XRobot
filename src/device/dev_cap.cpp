...
int8_t cap_init(cap_t *cap, const cap_param_t *param) {
...
int8_t cap_update(cap_t *cap, uint32_t timeout) {
  ASSERT(cap);
  can_rx_item_t rx;
  while (pdPASS ==
...
int8_t cap_control(cap_t *cap, cap_control_t *output) {
  ASSERT(cap);
  ASSERT(output);
...
int8_t cap_handle_offline(cap_t *cap) {
  ASSERT(cap);
...
int8_t cap_pack_ui(const cap_t *cap, ui_cap_t *ui) {
