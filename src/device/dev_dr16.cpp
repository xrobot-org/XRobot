...
static void dr16_rx_cplt_callback(void *arg) {
  ...
  xSemaphoreGiveFromISR(dr16->sem._new, &switch_required);
...
int8_t dr16_init(dr16_t *dr16) {
  ...
  dr16->sem._new = xSemaphoreCreateBinary();
...
bool dr16_wait_dma_cplt(dr16_t *dr16, uint32_t timeout) {
  return xSemaphoreTake(dr16->sem._new, pdMS_TO_TICKS(timeout)) == pdTRUE;
}
