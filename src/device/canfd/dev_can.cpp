...
bool Can::SendStdPack(bsp_can_t can, Pack& pack) {
  can_sem_[can]->Wait(UINT32_MAX);
  bool ans = bsp_can_trans_packet(can, CAN_FORMAT_STD_DATA, pack.index, pack.data) ==
             BSP_OK;
  can_sem_[can]->Post();
  return ans;
}
...
bool Can::SendExtPack(bsp_can_t can, Pack& pack) {
  can_sem_[can]->Wait(UINT32_MAX);
  bool ans = bsp_can_trans_packet(can, CAN_FORMAT_EXT_DATA, pack.index, pack.data) ==
             BSP_OK;
  can_sem_[can]->Post();
  return ans;
}
...
bool Can::SendFDStdPack(bsp_can_t can, uint32_t id, uint8_t* data,
  ...
  can_sem_[can]->Wait(UINT32_MAX);
  bool ans =
      bsp_canfd_trans_packet(can, CAN_FORMAT_STD_DATA, id, data, size) == BSP_OK;
  can_sem_[can]->Post();
  return ans;
}
...
bool Can::SendFDExtPack(bsp_can_t can, uint32_t id, uint8_t* data,
  ...
  can_sem_[can]->Wait(UINT32_MAX);
  bool ans =
      bsp_canfd_trans_packet(can, CAN_FORMAT_EXT_DATA, id, data, size) == BSP_OK;
  can_sem_[can]->Post();
  return ans;
}
