...
#define MOTOR_MAX_ROTATIONAL_SPEED (200.0f)

SpeedControl::SpeedControl(Param& param)
    : pid_(param.pid, 1000), motor_(param.motor, "motor") {
  /* 使用lambda创建控制线程函数 */
...
void SpeedControl::Control() {
  ...
  /* 计算PID */
  this->output_ =
      this->pid_.Calculate(this->speed_setpoint_,
                           this->motor_.GetSpeed() / MOTOR_MAX_ROTATIONAL_SPEED,
                           this->dt_) +
      0.057f;

  /* 电机输出 */
  this->motor_.ControlCurrent(this->output_);
}
